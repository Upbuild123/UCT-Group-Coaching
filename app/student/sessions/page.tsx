...
export default async function MySessionsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold tracking-tight text-slate-900 mb-6">My Sessions</h1>
      {(signups ?? []).length === 0 ? (
            ...
            return (
              <div key={signup.id} className="card">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-xs text-slate-400 mb-1">{group.rounds?.title}</p>
                    <h3 className="font-semibold text-slate-900">{group.users?.name}</h3>
                  ...
                  <span className={group.status === 'canceled' ? 'badge-red' : 'badge-green'}>
                    {group.status}
                  </span>
                </div>
              </div>
